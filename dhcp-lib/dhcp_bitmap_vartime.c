...
uint32_t dhcp_bmpool_var_peek(dhcp_bmpool_var_t *pool, uint32_t cur_time, uint32_t *lease_time) {
    ...
    uint32_t last_ip = dhcp_bm_last_ip(&(pool->counter), pool->pool_start, pool->range_size);
    if (pool->range->expire_time > 0 && dhcp_bm_range_full(pool->pool_start, last_ip, pool->range_size)) 
        return 0u;
    ...
    uint32_t candidate = dhcp_bm_next_ip(&pool->counter, pool->pool_start, pool->range_size);
    if (dhcp_bm_used(pool->range->ips, candidate - pool->pool_start, pool->range_size)) return 0u;
